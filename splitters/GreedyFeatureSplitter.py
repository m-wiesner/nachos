...
class GreedyFeatureSplitter(RandomFeatureSplitter):
    ...
    def split(self, recordings):
        ...
        for iter_num in tqdm(range(self.max_iter)):
            ...
            error = self.error(train, held_out)
            print("---------------------------------")
            print(f'i: {iter_num}, T: {train_ratio:0.2f}, H: {heldout_ratio:0.2f}, S: {error}')
            samples = self.draw_k_random_samples(feats, recordings_set)
            curr_sets, curr_complements = self.update_curr_sets(
                curr_sets, curr_complements, samples, error
            ) 
    ...
    def update_curr_sets(self, curr_sets, curr_complements, samples, error):
        best_error = error
        ...
        for i, f in enumerate(curr_sets):
            ...
            for s_idx, s in enumerate(samples):       
                ...
                error = self.error(train, held_out)
                if error < best_error and len(held_out) / len(self.fids) > self.heldout_min:
                    print(f"{i}, {s_idx}: {error} {best_error}")
                    best_error = error
                    best_update = [j for j in update]
                    best_complement = [j for j in complement]
        return best_update, best_complement
    
    def error(self, train, heldout):
        train_ratio = len(train) / len(self.fids)
        ...
        return train_error + heldout_error
